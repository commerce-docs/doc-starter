...
namespace :check do
  desc 'Optimize images in modified files, or by path (rake check:image_optim path=path/to/dir/or/file).'
  task :image_optim do
    ...
    puts 'Finished image optimization'.green
  end
  ...
  task :mdl do
    ...
    report = `bin/mdl #{path}`
    puts 'Finished checking Markdown'.green
    puts report.yellow
